...
public class ProductController {
    ...
    public ResponseEntity getAllProductsByCategory(@RequestParam Category category){

        List<ProductResponseDto> productResponseDtos = productService.getAllProductsByCategory(category);
        return new ResponseEntity(productResponseDtos,HttpStatus.FOUND);
    }

    // get all the products in a category who have price greater than or equals to 500
    @GetMapping("/getAllProductByCategoryByPriceGreater")
    public ResponseEntity getAllProductByCategoryByPriceGreater(@RequestParam Category category){

        List<ProductResponseDto> productResponseDtos = productService.getAllProductByCategoryByPriceGreater(category);
        return new ResponseEntity(productResponseDtos,HttpStatus.FOUND);
    }
